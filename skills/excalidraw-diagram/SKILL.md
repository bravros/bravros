...
### Step 0/6: Assess Depth Required
...
### Step 2/6: Map Concepts to Patterns
...
### Step 3/6: Ensure Variety
For multi-concept diagrams: **each major concept must use a different visual pattern**. No uniform cards or grids.

### Step 4/6: Sketch the Flow
Before JSON, mentally trace how the eye moves through the diagram. There should be a clear visual story.

### Step 5/6: Generate JSON
Only now create the Excalidraw elements. **See below for how to handle large diagrams.**

### Step 6/6: Render & Validate (MANDATORY)
...
After the diagram is complete and validated, ask the user if they want to open it in Excalidraw. Default to the public instance at **https://excalidraw.com**. If the user has a custom/self-hosted instance, use that instead.

```bash
EXCALIDRAW_URL="${EXCALIDRAW_URL:-https://excalidraw.com}"
if [[ "$OSTYPE" == "darwin"* ]]; then
  open "$EXCALIDRAW_URL"
else
  xdg-open "$EXCALIDRAW_URL" 2>/dev/null || echo "Open: $EXCALIDRAW_URL"
fi
```

Then tell the user to import the `.excalidraw` file via the menu (hamburger icon → Open) or by dragging it into the browser window.

**Default theme**: Dark mode is configured on the self-hosted instance.
