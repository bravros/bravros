...
PR number: `$ARGUMENTS` if numeric, else `PR=$(get-pr-info --json number -q .number)`. Non-numeric `$ARGUMENTS` are instructions for this round ("skip the docs nit", "also fix X"), never a PR number.
...
INTENT: land this feature — merge the PR into its base, record completion in `.workflow/events.jsonl`, route the homolog→main decision. Git/project operation only; never touches application code.
...
5. **Scaffold**: empty folder → `README.md` (`# {NAME}`) + `AGENT.md`; non-empty → `AGENT.md` only if missing. The AGENT.md branch-policy paragraph switches on `WANT_HOMOLOG` (direct-main text vs `feature/* → homolog → main`). `.bravros/config.json` from step 4 is always included.
...
- **The police staging lane — not a token — is what lets the merge through**: head `homolog`, `mergeStateStatus` `CLEAN`, no autonomous lock, plain `merge-pr 1234 --merge` with the **literal** PR number (no `cd … &&`, no `-R`/URL) — the hook reads raw command text, so a `$VAR` in the PR argument is unreadable → blocked. A repo with `police.staging_lane: reviewed` needs `bravros police unlock` from a separate terminal — say so ONCE, do not loop.
...
3. Push & merge to `homolog` → `create-pr --base main --head homolog --title "🩹 hotfix: <description>"` (body via `--body-file` written in a previous step).
4. **Wait for mergeability**, or the first merge hits the `UNKNOWN` block: `until [ "$(get-pr-info "$PR_NUMBER" --json mergeStateStatus -q .mergeStateStatus)" != "UNKNOWN" ]; do sleep 2; done`.
5. Check autopr gate → `merge-pr 1234 --merge > /tmp/bravros-merge-1234.txt 2>&1` (substitute the literal number — a `$VAR` here is unreadable to the hook; no `cd … &&`, no `| tail`) → verify state == `MERGED`. A `✋🏽 Police Block` names its reason — relay it in one line; never `gh api`/raw HTTP, never `/promote`.
6. Sync `homolog` from `main` (`git checkout homolog && git pull && git fetch origin main && git merge ...`).
7. Close plan if applicable (`.workflow/events.jsonl`) → `bravros commit`.
...
## Skill: bravros-laravel-cloud-ready
...
## Skill: bravros-laravel-pest-rector
...
Not `📋 plan:` — that type is for `.workflow/` bookkeeping and misfiles the change.
...
## Skill: bravros-laravel-pest5-upgrade
Upgrade a PHP/Laravel repo from Pest 4 to Pest 5 and enable Test Impact Analysis locally.
...
3. **Branch gate — never orchestrate product code on the staging or main branch.** Before the first
   ...
   branch or inside a worktree → stay there, never switch. Only `.workflow/` bookkeeping (id
...
5. **Per-unit loop**: dispatch → haiku verifier runs ONLY targeted tests → review the diff yourself →
   ...
   useful by your next turn → the task stop tool and work from the partials. "Never the full suite" is a
...
- **Creating the PR is not the end of the task — the review trigger is.** `/pr` is one unit of
  ...
  Stopping after `create-pr` leaves the job half done, so do not report success until the
...
1. **Write the body with the Write tool** to an absolute path — the scratchpad (`<scratchpad>/pr-body-<branch>.md`) by default; `<repo>/.workflow/pr-body-<branch>.md` only when no scratchpad exists, deleted after the PR opens (`/recon` commits `.workflow/` wholesale) — with Summary, Changes, Technical Notes, Test Plan, References. Then re-read it and strip any AI-attribution footer.
2. `create-pr --base "$BASE" --title "<emoji> <type>: <title>" --body-file <abs path>` as its own command. **Never** a heredoc, `cat > f && create-pr`, or `cp … && create-pr --body-file` in the same command: the police hook validates the body file **before** the command runs and blocks an unreadable one (3 blocks in a row on paylog, 2026-09-11 06:53).
...
`/orchestrate .workflow/P-NNNN-<slug>/`, and name the branch it will cut from the staging branch —
...
## Skill: bravros-start
EXPLICIT-INVOCATION ONLY — trigger only when the user types /start. Initializes a new project with stack-aware AGENT.md, .bravros/config.json, .gitignore, and base structure. Do NOT trigger on natural-language phrases like init or setup without the slash.
...
4. **Step 3 — Ledger + close out:** Write `.workflow/sweep-ledger.md` and announce completion via `announce.sh`.
