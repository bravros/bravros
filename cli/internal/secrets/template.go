...
type TemplateOpts struct {
	OutputFile string    // default: ~/.config/kaisser/secrets-template.env
...
func ExportTemplate(opts TemplateOpts) (string, error) {
	...
	sb.WriteString("# kaisser secrets template\n")
	...
	if outFile == "" {
		...
		outFile = filepath.Join(home, ".config", "kaisser", "secrets-template.env")
	}
