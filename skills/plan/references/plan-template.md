...
**Verify:** `<test-command> --filter="OtherTest"`
...
**Verify:** `<test-command> --filter="FullScope"`
...
## CLI Commands for Plan Lifecycle

All plan operations use `bravros` commands. These are the commands available at each stage:

| Stage | Command | Purpose |
|-------|---------|---------|
| Create | `bravros meta --reserve` | Get project, base branch, plan file + atomically reserve IDs |
| Create | `bravros branch create --checkout-only` | Sync base branch (no new branch) |
| Create | `bravros commit "msg"` | Stage .planning/ + commit |
| Review | `bravros branch create <name>` | Create feature branch |
| Execute | `bravros commit "msg" [files]` | Stage + format + commit |
| Check | `bravros plan-tasks [--diff auto]` | Extract/diff task lists |
| Check | `bravros plan-check-status` | Verify plan-check was run |
| Check | `bravros context --diffs` | Git context for PR |
| Finish | `bravros finish [--pr N] [--dry-run]` | Atomic plan completion |
| Finish | `bravros ci-check` | CI workflow detection |
| Deploy | `bravros deploy [--dry-run]` | Deploy config to ~/.claude/ |

Note: `bravros meta` returns `base_branch` (CLI field name). In plan frontmatter, use `base` instead.

## CLI Commands Reference

| Command | Purpose |
|---------|---------|
| `bravros meta` | Plan metadata as JSON |
| `bravros meta --field stack.test_runner` | Get project's test runner |
| `bravros detect-stack` | Detect project stack and framework |
| `bravros detect-stack --versions` | Include package version details |
| `bravros context --diffs` | Git context with diffs for PR/plan-check |
| `bravros commit "<msg>"` | Commit with pint formatting |
| `bravros backlog` | List backlog items |
| `bravros plan-tasks` | List plan tasks as JSON |
| `bravros ci-check` | Check CI workflow status |
| `bravros finish` | Mark plan complete and archive |
| `bravros worktree setup <branch>` | Create isolated worktree |
| `bravros merge-pr <number>` | Merge PR with conflict resolution |
| `bravros init` | Initialize project with SDLC structure |
