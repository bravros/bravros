...
def deduplicate_entities(
    ...
    *,
    ...
    # Patch (afterpay fork): key by (label, source_file) instead of label only.
    # Otherwise generic method labels like ".__construct()" / ".handle()" / ".casts()"
    # collapse across all classes in the codebase into a single fake god-node.
    # source_file is a stable per-class disambiguator under PSR-4 / one-class-per-file.
