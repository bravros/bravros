...
// --- ParseComposerJSON (P-0122 — manifest fallback) ---

// TestParseComposerJSON_LivewireConstraint covers the Coordinator's repro —
// composer.json with livewire 3.x and no composer.lock should still surface
// `versions.livewire`. Was returning empty before P-0122.
func TestParseComposerJSON_LivewireConstraint(t *testing.T) {
	dir := setupTestDir(t)
	composerPath := filepath.Join(dir, "composer.json")
	writeFile(t, composerPath, `{
		"require": {
	...
	got, err := ParseComposerJSON(composerPath)
	if err != nil {
		t.Fatal(err)
	}
	if got["laravel"] != "10.0" {
		t.Errorf("expected laravel=10.0, got %q", got["laravel"])
	}
	if got["livewire"] != "3.0" {
		t.Errorf("expected livewire=3.0, got %q", got["livewire"])
	}
}

func TestParseComposerJSON_RequireDev(t *testing.T) {
	dir := setupTestDir(t)
	composerPath := filepath.Join(dir, "composer.json")
	writeFile(t, composerPath, `{
	...
	if got["pest"] != "2.0" {
		t.Errorf("expected pest=2.0 from require-dev, got %q", got["pest"])
	}
}

func TestParseComposerJSON_MissingFileReturnsEmpty(t *testing.T) {
	got, err := ParseComposerJSON("/nonexistent/composer.json")
	if err != nil {
		t.Fatalf("expected no error for missing file, got %v", err)
	}
	if len(got) != 0 {
...
func TestParseComposerJSON_DependencyNotInInterestingList(t *testing.T) {
	dir := setupTestDir(t)
	composerPath := filepath.Join(dir, "composer.json")
	writeFile(t, composerPath, `{
	...
	if len(got) != 0 {
		t.Errorf("expected empty map for non-interesting dep, got %v", got)
	}
}

// --- ParsePackageJSON (P-0122 — manifest fallback) ---

// TestParsePackageJSON_DaisyUIConstraint mirrors the rubric ask: package.json
// with daisyui ^4 should surface `versions.daisyui` even when no
// package-lock.json exists (typical for pnpm/bun/yarn / pre-install).
func TestParsePackageJSON_DaisyUIConstraint(t *testing.T) {
	dir := setupTestDir(t)
	...
	got, err := ParsePackageJSON(pkgPath)
	if err != nil {
		t.Fatal(err)
	}
	cases := map[string]string{
	...
	for k, want := range cases {
		if got[k] != want {
			t.Errorf("ParsePackageJSON[%q] = %q, want %q", k, got[k], want)
		}
	}
}

func TestParsePackageJSON_PeerDependencies(t *testing.T) {
	dir := setupTestDir(t)
	pkgPath := filepath.Join(dir, "package.json")
	writeFile(t, pkgPath, `{
	...
	if got["react"] != "18.0.0" {
		t.Errorf("expected react=18.0.0 from peerDependencies, got %q", got["react"])
	}
}

func TestParsePackageJSON_SkipsOptionalDependencies(t *testing.T) {
	dir := setupTestDir(t)
	pkgPath := filepath.Join(dir, "package.json")
	writeFile(t, pkgPath, `{
		"dependencies": {
	...
	if got["react"] != "18.0.0" {
		t.Errorf("expected react=18.0.0, got %q", got["react"])
	}
	for _, key := range []string{"tailwindcss", "prisma"} {
		if _, ok := got[key]; ok {
			t.Errorf("optional dependency %q should not be reported as installed/present; got %v", key, got)
		}
	}
}

func TestParsePackageJSON_MissingFileReturnsEmpty(t *testing.T) {
	got, err := ParsePackageJSON("/nonexistent/package.json")
	if err != nil {
		t.Fatalf("expected no error for missing file, got %v", err)
	}
	if len(got) != 0 {
...
func TestParsePackageJSON_DependencyNotInInterestingList(t *testing.T) {
	dir := setupTestDir(t)
	pkgPath := filepath.Join(dir, "package.json")
	writeFile(t, pkgPath, `{
...
// TestParseComposer_LockfileWinsOverManifest verifies the merge order in
// detect.go: when both composer.lock and composer.json exist, the lockfile's
// resolved version overrides the manifest's constraint version (because
// detect.go applies manifest first, then lockfile).
func TestParseComposer_LockfileWinsOverManifest(t *testing.T) {
	dir := setupTestDir(t)
	manifestPath := filepath.Join(dir, "composer.json")
	lockPath := filepath.Join(dir, "composer.lock")
	writeFile(t, manifestPath, `{"require": {"livewire/livewire": "^3.0"}}`)
	writeFile(t, lockPath, `{"packages": [{"name": "livewire/livewire", "version": "v3.4.12"}]}`)

	manifestVersions, _ := ParseComposerJSON(manifestPath)
	lockVersions, _ := ParseComposerLock(lockPath)
	...
	if merged["livewire"] != "3.4.12" {
		t.Errorf("expected lockfile to override manifest (livewire=3.4.12), got %q", merged["livewire"])
	}
}
