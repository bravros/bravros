...
<!-- announce-template: "Removidas {N} ramificações obsoletas. Ramo {BRANCH}, projeto {PROJECT}." -->
Fire after the final step when deletion count > 0:

```bash
bash ~/.agent_config/scripts/announce.sh --force "Removidas $DELETED_COUNT ramificações obsoletas. Ramo $(git branch --show-current), projeto $(basename "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")")." studio || true
```
