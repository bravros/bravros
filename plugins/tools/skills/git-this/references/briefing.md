...
2. **Collision** → announce (below), propose 3 free alternatives (suffix, year, or a name from package metadata / README H1), pick via AskUserQuestion; re-sanitize "Other" input; loop max 3.
...
```bash
bash ~/.claude/scripts/announce.sh "Nome do repositório já existe, aguardando sua escolha de alternativa. Projeto $(basename "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")")." studio >/dev/null 2>&1 || true
```
