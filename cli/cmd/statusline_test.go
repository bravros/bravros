...
// ─── Golden tests: statusline output is FROZEN ─────────────────────────────────
//
// These pin the exact byte-for-byte output of assembleOutput (ANSI codes
// included) for representative inputs. The operator froze the statusline
// format after 3 breakages on record: if one of these fails, the fix is
// reverting your change — update the golden strings only for a deliberate,
// operator-requested format change.

func TestStatuslineGoldenNormalUsage(t *testing.T) {
	input := statuslineInput{
		Model: statuslineModel{DisplayName: "Opus"},
		Workspace: statuslineWorkspace{
			CurrentDir: "/Users/me/proj",
			ProjectDir: "/Users/me/proj",
		},
		ContextWindow: statuslineContextWin{
			CurrentUsage:      statuslineUsage{InputTokens: 420000},
			ContextWindowSize: 1000000,
		},
		Cost: statuslineCost{TotalDurationMs: 240000},
	}
	got := assembleOutput(input, "main", "+10/-2", "", "")
	want := "\033[35mOpus\033[0m" +
		" \033[2m│\033[0m \033[32m████░░░░░░\033[0m \033[36m42%\033[0m" +
		" \033[2m~5m left\033[0m" +
		" \033[2m│\033[0m \033[33m4m\033[0m" +
		" \033[2m│\033[0m \033[32mmain\033[0m" +
		" \033[33m+10/-2\033[0m" +
		" \033[2m│\033[0m \033[34mproj\033[0m"
	if got != want {
		t.Errorf("golden drift (normal usage):\n got: %q\nwant: %q", got, want)
	}
}

func TestStatuslineGolden100PercentEdge(t *testing.T) {
	input := statuslineInput{
		Model: statuslineModel{DisplayName: "Opus"},
		ContextWindow: statuslineContextWin{
			CurrentUsage:      statuslineUsage{InputTokens: 1000000},
			ContextWindowSize: 1000000,
		},
		Cost: statuslineCost{TotalDurationMs: 600000},
	}
	got := assembleOutput(input, "", "", "", "")
	// At 100%: full red bar, no "~left" estimate (remaining <= 0).
	want := "\033[35mOpus\033[0m" +
		" \033[2m│\033[0m \033[31m██████████\033[0m \033[36m100%\033[0m" +
		" \033[2m│\033[0m \033[33m10m\033[0m"
	if got != want {
		t.Errorf("golden drift (100%% edge):\n got: %q\nwant: %q", got, want)
	}
}

func TestStatuslineGoldenEmptyInput(t *testing.T) {
	got := assembleOutput(statuslineInput{}, "", "", "", "")
	// Empty/missing data: model defaults to "Claude", empty green bar, 0%.
	want := "\033[35mClaude\033[0m" +
		" \033[2m│\033[0m \033[32m░░░░░░░░░░\033[0m \033[36m0%\033[0m"
	if got != want {
		t.Errorf("golden drift (empty input):\n got: %q\nwant: %q", got, want)
	}
}

func TestStatuslineGoldenFullFeatures(t *testing.T) {
	input := statuslineInput{
		Model:       statuslineModel{DisplayName: "Sonnet"},
		Exceeds200K: true,
		Worktree:    statuslineWorktree{Name: "wt-x"},
		Workspace: statuslineWorkspace{
			CurrentDir: "/a/b",
			ProjectDir: "/a",
		},
		ContextWindow: statuslineContextWin{
			CurrentUsage:      statuslineUsage{InputTokens: 700000},
			ContextWindowSize: 1000000,
		},
		Cost: statuslineCost{TotalDurationMs: 3660000},
	}
	got := assembleOutput(input, "feat/x ↑2", "+1/-1", "2x", ansiGreen)
	// 70% red bar, 200K+ warning, promo, worktree name overrides branch,
	// code stats ride branch without separator, subdir folder display.
	want := "\033[35mSonnet\033[0m" +
		" \033[2m│\033[0m \033[31m███████░░░\033[0m \033[36m70%\033[0m" +
		" \033[31m\033[1m200K+\033[0m" +
		" \033[2m~26m left\033[0m" +
		" \033[2m│\033[0m \033[32m\033[1m2x\033[0m" +
		" \033[2m│\033[0m \033[33m1h1m\033[0m" +
		" \033[2m│\033[0m \033[32mwt-x\033[0m" +
		" \033[33m+1/-1\033[0m" +
		" \033[2m│\033[0m \033[34ma/b\033[0m"
	if got != want {
		t.Errorf("golden drift (full features):\n got: %q\nwant: %q", got, want)
	}
}
