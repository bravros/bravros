...
// .kaisser.yml:skills.enabled. When non-empty, `kaisser deploy` only copies
...
//
// Resolution chain for the .kaisser.yml file:
//  1. cwd (.kaisser.yml in the current working directory)
//  2. $KAISSER_PORTABLE_REPO env var (the canonical source repo path)
//  3. $HOME (the user's home directory)
//
// Returns an empty slice when no .kaisser.yml is found, when the config has
...
// loadPreserveConfig tries to load .kaisser.yml from the resolution chain,
...
func loadPreserveConfig() (*BravrosConfig, bool) {
	...
	// 2. $KAISSER_PORTABLE_REPO
	if repo := os.Getenv("KAISSER_PORTABLE_REPO"); repo != "" {
...
// loadFromDir attempts to parse .kaisser.yml from the given directory.
