...
- Creates branch from base branch (staging_branch from .bravros/config.json)
...
- Merge-checked destroy: refuses when the branch is not merged into
  origin/<base> (--base, default: staging_branch from .bravros/config.json) unless
...
func init() {
	...
	worktreeSetupFullCmd.Flags().StringVar(&worktreeSetupFullBase, "base", "", "Base branch (e.g. origin/homolog). Default: staging_branch from .bravros/config.json")
	...
	worktreeCleanupCmd.Flags().StringVar(&worktreeCleanupBase, "base", "", "Base branch to verify merge status against (default: staging_branch from .bravros/config.json)")
