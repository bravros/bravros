...
ssh_base_opts() {
    printf '%s' "-o StrictHostKeyChecking=accept-new -o ConnectTimeout=10 -o ServerAliveInterval=30 -o ServerAliveCountMax=3"
}
...
fetch_prod_versions() {
    PROD_MYSQL_VERSION=$(remote_query 'SELECT VERSION()' 2>/dev/null | head -1)
    ...
    PROD_REDIS_VERSION=$(cat <<'REMOTE_VER' | remote_bash 2>/dev/null | tail -1
...
fetch_local_versions() {
    LOCAL_MYSQL_VERSION=$(mysql_local -N -B -e 'SELECT VERSION()' 2>/dev/null | head -1)
    ...
    if [ -n "${REDIS_CLI_BIN:-}" ] && [ -n "${TARGET_REDIS_HOST:-}" ]; then
        ...
        out=$(printf '%s' "$out" | tr -d '\r' | sed -n 's/^redis_version://p' | head -1)
...
connect_production() {
    ...
    local missing
    missing=$(echo 'for b in mysqldump zstd; do command -v $b >/dev/null 2>&1 || echo $b; done' | remote_bash 2>/dev/null)
    if [ -n "$missing" ]; then
    ...
    for attempt in 1 2; do
        ...
        (
            while true; do
                ...
                tables=$(mysql_local -N -B -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = '${LOCAL_DB_NAME}';" 2>/dev/null | head -1)
...
case $MODE in
    ...
    "restore")
        if [ -n "$BACKUP_FILE" ]; then
            ...
            BACKUP_FILE=$(list_backup_files | head -1)
