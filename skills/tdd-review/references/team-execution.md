...
**Sonnet** for `[H]` and `[S]` workers and `[S]` team-leads. **Opus** for `[O]` workers, `[O]` team-leads, AND test-writing tasks (test quality impacts regression safety). **Coordinator = Sonnet** (5x more quota, sufficient for dispatch-verify-commit). **Haiku** = fallback only, never a default.
