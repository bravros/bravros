...
AskUserQuestion — "Which 1Password auth mode for this machine?": `desktop` (interactive,
...
```bash
bash ~/.bravros/scripts/announce.sh "Configuração de área de trabalho do 1Password necessária. Siga as instruções no terminal separado. Projeto $(basename "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")")." studio >/dev/null 2>&1 || true
```
...
```bash
bash ~/.bravros/scripts/announce.sh "Configuração de conta de serviço do 1Password necessária. Siga as instruções no terminal separado. Projeto $(basename "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")")." studio >/dev/null 2>&1 || true
```
