...
**8a — refresh hooks:** copy canonical files from the deployed `~/.agent_config/scripts/`, write three delegators, set `core.hooksPath`:

```bash
mkdir -p scripts/graphify .githooks
cp ~/.agent_config/scripts/graphify-refresh-hook.sh          scripts/graphify-refresh-hook.sh
cp ~/.agent_config/scripts/graphify/apply-labels.py          scripts/graphify/apply-labels.py
cp ~/.agent_config/scripts/graphify/strip-framework-verbs.py scripts/graphify/strip-framework-verbs.py
